...
(function () {
    ...
    var hasCaption = function (image) {
      // return image.parentNode !== null && image.parentNode.nodeName === 'FIGURE';
        return null !== image.parentNode && image.parentNode.classList.contains("img-lazy-wrap") ? '1' : (null !== image.parentNode && image.parentNode.classList.contains("img-native-wrap") ? 'wrap' : '');
    };
    ...
    var wrapInFigure = function (image, data) {
      // var figureElm = DOM.create('figure', { class: 'image' });
      // var figureElm = DOM.create('span', { class: 'image' });
      ...
      // figureElm.contentEditable = 'false';
        ...
        var figureElm,
            ...
            h = data.height,
            ratio = h * 100 / w,
            p = +(Math.round(ratio + "e+3")  + "e-3");

        image.setAttribute("loading", "lazy"),
            image.setAttribute("width", w),
            image.setAttribute("height", h),
        'wrap' === data.caption && ((image.setAttribute('loading', 'lazy'),
            image.setAttribute("src", imgurl),
            image.removeAttribute("data-src"),
            image.classList.add("img-fluid")),
            (figureElm = DOM.create("span", { class: "img-native-wrap", style: "padding-bottom:" + p + "%" })),
            DOM.insertAfter(figureElm, image),
            figureElm.appendChild(image)),
        '' === data.caption && (image.setAttribute("src", imgurl), image.removeAttribute("data-src"));
    };
    ...
    var removeFigure = function (image, data) {
        var figureElm = image.parentNode,
            imgurl = image.getAttribute("data-src");
        imgurl || (imgurl = image.getAttribute("src")),
        null !== image.parentNode && image.parentNode.classList.contains("img-lazy-wrap") && (DOM.insertAfter(image, figureElm), DOM.remove(figureElm)),
        null !== image.parentNode && image.parentNode.classList.contains("img-native-wrap") && (DOM.insertAfter(image, figureElm), DOM.remove(figureElm)),
            image.classList.contains("media-lazy") && image.classList.contains("lazyload") ? image.classList.remove("media-lazy", "lazyload") : image.classList.remove("lazyload"),
            image.setAttribute("src", imgurl),
    ...
    var toggleCaption = function (image, data) {
        if (hasCaption(image)) {
            removeFigure(image, data);
        } else {
            wrapInFigure(image, data);
        }
    };
    ...
    var create = function (normalizeCss, data) {
        var image = document.createElement('img');
        write(normalizeCss, __assign(__assign({}, data), { caption: '' }), image);
        setAlt(image, data.alt, data.isDecorative);
        image.setAttribute("loading", "lazy");
        if (data.caption === 'wrap') {
            var w = data.width ? data.width : 1,
                h = data.height ? data.height : 1,
                ratio = h*100/w,
                p = +(Math.round(ratio + "e+3")  + "e-3");

            var figure = DOM.create('span', { class: 'img-native-wrap', style: "padding-bottom:" + p + "%" });
            figure.appendChild(image);
            return figure;
        } else {
            image.classList.add("img-fluid");
    ...
    var write = function (normalizeCss, newData, image) {
      ...
      updateProp(image, oldData, newData, 'caption', function (image, _name, _value) {
        return toggleCaption(image, newData);
      });
    ...
    var makeItems = function (info) {
      ...
      var caption = {
          ...
          items: [
              { text: 'Native lazy load - inline', value: '' },
              { text: 'Native lazy load - with placeholder', value: 'wrap' }
          ]
      };
