    ...
    return function (config, element) {
        ...
        var toogleCm = function (elem, mode, show, options, mceHeight) {
            if (!show) {
                ...
                editor = cnt.find('div.mce-tinymce') || cnt.find('div.tox-tinymce');
                if (editor.length) {
                    editor.show();
                    $(elem).hide();
                    return;
        ...
        var activedCM = function (elem, mode, options) {
            ...
            var mce5 = cnt.find('.tox-tinymce');

            if (mceHeight < 300) {
                mceHeight = 300;
            }

            var isMceActive = mceE.length || mce4.length || mce5.length;
